...
static lv_color_t battery_image_buffer[ZMK_SPLIT_BLE_PERIPHERAL_COUNT + SOURCE_OFFSET][20 * 12];
...
static void draw_battery(lv_obj_t *canvas, uint8_t level, bool usb_present) {
    ...
    lv_canvas_draw_rect(canvas, 1, 1, 8, 3, &rect_fill_dsc);
...
int zmk_widget_dongle_battery_status_init(struct zmk_widget_dongle_battery_status *widget, lv_obj_t *parent) {
    ...
    for (int i = 0; i < ZMK_SPLIT_BLE_PERIPHERAL_COUNT + SOURCE_OFFSET; i++) {
        ...
        lv_canvas_set_buffer(image_canvas, battery_image_buffer[i], 10, 5, LV_IMG_CF_TRUE_COLOR);
