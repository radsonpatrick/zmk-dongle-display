...
static void draw_battery(lv_obj_t *canvas, uint8_t level, bool usb_present) {
    ...
    if (usb_present) {
        ...
        rect_fill_dsc.border_width = 3;
    }

    lv_canvas_set_px(canvas, 18, 0, lv_color_white());
    lv_canvas_set_px(canvas, 18, 1, lv_color_white());
    lv_canvas_set_px(canvas, 19, 0, lv_color_white());
    lv_canvas_set_px(canvas, 19, 1, lv_color_white());
    lv_canvas_set_px(canvas, 18, 8, lv_color_white());
    lv_canvas_set_px(canvas, 18, 9, lv_color_white());
    lv_canvas_set_px(canvas, 19, 8, lv_color_white());
    lv_canvas_set_px(canvas, 19, 9, lv_color_white());



    if (level <= 10 || usb_present) {
        lv_canvas_draw_rect(canvas, 16, 1, -16, 8, &rect_fill_dsc);
    } else if (level <= 30) {
        lv_canvas_draw_rect(canvas, 16, 1, -10, 8, &rect_fill_dsc);
    } else if (level <= 50) {
        lv_canvas_draw_rect(canvas, 16, 1, -8, 8, &rect_fill_dsc);
    } else if (level <= 70) {
        lv_canvas_draw_rect(canvas, 16, 1, -6, 8, &rect_fill_dsc);
    } else if (level <= 90) {
        lv_canvas_draw_rect(canvas, 16, 1, -2, 8, &rect_fill_dsc);
    }
}
